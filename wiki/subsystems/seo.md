...
(`schema.ts:174`, emits `BlogPosting`), `getBreadcrumbSchema` (`schema.ts:216`), and `serializeJsonLd`
(`schema.ts:234`). Stable `@id`s (`organizationId`, `schema.ts:49`) let nodes cross-reference inside a
...
> [!note] `serializeJsonLd` escapes `<` → `<` (`schema.ts:240`) so a value containing `</script>`
...
- **Sitemap** — `@astrojs/sitemap` emits `/sitemap-index.xml` (linked in `BaseHead`). A `filter`
  (`astro.config.mjs:50`) drops `noindex` routes (the dev-only `/examples` catalog and 404s), so the
  sitemap never contradicts a page's `noindex`.
...
- **Trailing slashes** — one URL shape, now enforced by config: `trailingSlash: "always"`
  (`astro.config.mjs:14`), agreeing with the directory build, `getLocalizedRoute`, canonical,
  hreflang, and OG. It had been loosened to `"ignore"` only because `"always"` 404'd Keystatic's
  extensionless API calls; [[subsystems/keystatic-cms|removing Keystatic]] (2026-07-17) allowed
  tightening it back.

## What's deliberately deferred (YAGNI)

The blog collection ([[subsystems/content-collections]]) exists but has **no route yet**, so RSS and a
visible breadcrumb component aren't built — a feed/breadcrumb for non-existent pages would be premature.
When the blog route lands: add a dependency-free per-locale `rss.xml.ts` (hand-rolled, escaped RSS 2.0,
like everything else in `head`), pass `getArticleSchema` + `getBreadcrumbSchema` + the `article` prop
from the post page, and make `heroImage` required so every post has an OG image. See `.claude/rules/seo.md`.
...
`astro.config.mjs` (still `https://example.com`) feeds **all** of it — canonical, OG, hreflang, sitemap,
robots, llms — so setting it once before deploy fixes every absolute URL at once.
